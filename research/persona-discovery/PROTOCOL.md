...
For each selected feature, generate positive and negative additive-steering responses on 12 disjoint development prompts, plus shared unsteered responses. The direction has unit norm and the intervention magnitude is 10% of the median unsteered residual norm from discovery. Interventions apply at the layer output during both prompt prefill and autoregressive decoding. This single magnitude is an initial causal screen, not the final optimized treatment. Save every output, including incoherent behavior. Sampling seeds and generation configuration are recorded.
...
The question is non-executable in Afterlight's existing API evaluator. Design in Auto Labs preserves its identity and source. The dedicated study page receives measured progress from Kaggle notebook logs through an authenticated relay on the owner computer. The relay runs for a bounded observation window; Kaggle execution continues independently if that computer sleeps. Coarse job status and measured notebook counts remain distinct, with stale updates labeled. Completing discovery does not complete confirmation.
